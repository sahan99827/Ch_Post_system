...
function numberToWords(num) {
  ...
  if (integerPart >= 1000000) {
    result +=
      convertLessThanThousand(Math.floor(integerPart / 1000000)) + " Million ";
    integerPart %= 1000000;
  }
  if (integerPart >= 1000) {
    result +=
      convertLessThanThousand(Math.floor(integerPart / 1000)) + " Thousand ";
    integerPart %= 1000;
  }
  if (integerPart > 0) {
...
document.getElementById("customerName").textContent = customerName;
    ...
    document.getElementById("invoiceTime").textContent =
    ...
    document.getElementById("subTotal").textContent =
      (invoice.subtotal || 0).toFixed(2) + " LKR";
    document.getElementById("taxAmount").textContent =
      (invoice.tax || 0).toFixed(2) + " LKR";
    document.getElementById("totalAmount").textContent =
      (invoice.total || 0).toFixed(2) + " LKR";

    // Set amount in words
    document.getElementById("amountInWords").textContent = numberToWords(
      invoice.total || 0
    );
